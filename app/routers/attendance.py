...
def mark_attendance(item: schemas.AttendanceCreate, db: Session = Depends(get_db)):
...
def bulk_mark(items: list[schemas.AttendanceCreate], db: Session = Depends(get_db)):
...
def upsert_day(body: schemas.AttendanceUpsertDay, db: Session = Depends(get_db)):
...
def update_record(
    record_id: int, item: schemas.AttendanceCreate, db: Session = Depends(get_db)
):
