...
export class AttendanceComponent {
  ...
  /** Teacher with a class assigned on faculty profile — class picker locked to that class. */
  get isTeacherPortal(): boolean {
    return (
      this.session.isLoggedInAs('teacher') &&
      !!this.session.teacherClassAssigned().trim()
    );
  }

  constructor() {
    const tc = this.session.teacherClassAssigned().trim();
    const isTeacher = this.session.isLoggedInAs('teacher');
    if (isTeacher && tc) {
      this.bulkClass = tc;
      this.regClass = tc;
      this.calClass = tc;
    ...
    const studentsReq =
      isTeacher && tc
        ? this.api.listStudents({ className: tc })
  ...
  setView(v: AttendanceView): void {
    if (v === 'holidays' && this.session.isLoggedInAs('teacher')) {
      v = 'class_bulk';
    }
    this.view = v;
    if (v === 'holidays') {
