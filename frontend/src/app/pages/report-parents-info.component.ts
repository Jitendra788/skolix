...
type PColKey = 'sr' | 'admissionId' | 'studentName' | 'fatherName' | 'motherName' | 'parentPhone' | 'classCol' | 'address';

const PCOL_META: { key: PColKey; label: string }[] = [
  { key: 'sr', label: 'Sr' },
...
@Component({
  selector: 'app-report-parents-info',
  standalone: true,
  imports: [CommonModule, FormsModule],
  template: `
    <section class="rep-page">
      <header class="rep-head">
        <h1>Parents info report</h1>
        <p>Guardian contact details by class (same filters as students info report).</p>
      </header>

      <div class="rep-toolbar">
        <div class="rep-filters">
          <select class="rep-select" [ngModel]="className()" (ngModelChange)="className.set($event)">
        ...
        <div class="rep-actions no-print">
          ...
          <div class="rep-dd">
            ...
            <div class="rep-dd-panel">
              @for (c of PCOL_META; track c.key) {
                <label class="rep-cb">
          ...
          <label class="rep-search">
            <span>Search:</span>
            <input type="search" [ngModel]="searchQuery()" (ngModelChange)="onSearch($event)" placeholder="Filter..." />
          </label>
        </div>
      </div>

      @if (loadError()) {
        <p class="rep-err">Could not load students.</p>
      }
...
export class ReportParentsInfoComponent {
  readonly PCOL_META = PCOL_META;
  ...
  readonly colVisible = signal<Record<PColKey, boolean>>({
    sr: true,
    admissionId: true,
    studentName: true,
    fatherName: true,
  ...
  readonly visibleColMeta = computed(() => PCOL_META.filter((c) => this.colVisible()[c.key]));
  ...
  toggleCol(key: PColKey, ev: Event): void {
  ...
  cellValue(key: PColKey, s: Student, serialNo: number): string {
    switch (key) {
      case 'sr':
  ...
  copyExport(): void {
    void navigator.clipboard.writeText(this.buildTsv());
  }
  ...
  csvExport(): void {
    ...
    this.downloadBlob([header, ...lines].join('\n'), 'parents-info-report.csv', 'text/csv;charset=utf-8');
  }

  excelExport(): void {
    this.downloadBlob(this.buildTsv(), 'parents-info-report.xls', 'application/vnd.ms-excel;charset=utf-8');
  }
