...
def queue_broadcast(
    ...
    status = item.status.strip() if item.status else "sent_simulated"
    ...
    row = models.WhatsAppBroadcast(
        ...
        parent_phone=item.parent_phone.strip(),
    )
    db.add(row)
    db.commit()
    db.refresh(row)
...
def get_whatsapp_config():
    return {
        "mode": "stub",
        "sms_provider": "Set SMS_PROVIDER=textlocal|fast2sms|twilio|android_gateway for real SMS delivery",
        "hint": "Set WHATSAPP_API_TOKEN and WHATSAPP_PHONE_ID env vars and wire Meta Cloud API in app/services/whatsapp.py",
    }
