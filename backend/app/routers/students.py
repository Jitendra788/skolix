...
def _student_self_guard(
    ...
    if not payload or str(payload.get("role", "")) != "student":
        raise HTTPException(status_code=403, detail="Student access only")
    if str(payload.get("sub", "")) != str(student_id):
...
@router.patch("/{student_id}/self-profile", response_model=schemas.StudentRead)
def patch_student_self_profile(
    student_id: int,
    item: schemas.StudentSelfProfilePatch,
    db: Session = Depends(get_db),
    _auth: None = Depends(_student_self_guard),
):
    """Student updates their own contact / display fields (Bearer JWT, role=student, sub=id)."""
    row = db.get(models.Student, student_id)
    if not row:
        raise HTTPException(status_code=404, detail="Not found")
    data = item.model_dump(exclude_unset=True)
    photo = data.pop("profile_photo_url", None)
    if photo is not None:
        ex = _parse_admission_extras(row.admission_extras)
        url = (photo or "").strip()[:2000]
        if url:
            ex["portal_photo_url"] = url
        else:
            ex.pop("portal_photo_url", None)
        row.admission_extras = _serialize_admission_extras(ex)
    if "full_name" in data and data["full_name"] is not None:
        row.full_name = (data["full_name"] or "").strip()
    if "parent_phone" in data and data["parent_phone"] is not None:
        row.parent_phone = (data["parent_phone"] or "").strip()
    if "parent_name" in data and data["parent_name"] is not None:
        row.parent_name = (data["parent_name"] or "").strip()
    if "date_of_birth" in data and data["date_of_birth"] is not None:
        row.date_of_birth = (data["date_of_birth"] or "").strip()
    if "gender" in data and data["gender"] is not None:
        row.gender = (data["gender"] or "").strip()
