...
from .auth_context import parse_bearer_payload


def _reject_teacher_write(authorization: str | None) -> None:
    p = parse_bearer_payload(authorization)
    if p and str(p.get("role", "")) == "teacher":
        raise HTTPException(
            status_code=403, detail="Teachers cannot modify the holiday calendar"
        )
...
def create_holiday(
    item: schemas.SchoolHolidayCreate,
    authorization: str | None = Header(default=None),
    db: Session = Depends(get_db),
):
    _reject_teacher_write(authorization)
...
def generate_weekly_holidays(
    item: schemas.SchoolHolidayRecurringCreate,
    authorization: str | None = Header(default=None),
    db: Session = Depends(get_db),
):
    _reject_teacher_write(authorization)
